...
for file in test_files:
    ...
    model.fit(file['Value1'])
    y_pred = model.predict(file['Value1'])

    y_pred_groups = Benchmarking.create_anomaly_groups(pd.Series(y_pred),
                                                       show_printout=False)

    group_acc = Benchmarking.evaluate_model(file['Labels'],y_pred,
                                            show_printout=False)
    group_accs.append(group_acc.loc['Group Accuracy']['Score'])

    results[f'File {idx}'] =(y_pred_groups, group_acc)
    idx += 1
```
...
for idx , metrics in results.items():
    col = pd.Series(metrics[0],name=idx)
    anoms = pd.concat([anoms, col], axis=1)
    
groups = ['Anomaly' for _ in range(1,12)] +['Group Accuracy']
row_names = [f'Anomaly {idx}' for idx in range(1,12)] + ['Group']
print(len(groups))
print(len(anoms))

anoms.loc[11] = group_accs
anoms['grp'] = groups
anoms['row'] = row_names
...
(
    GT(anoms)
    .tab_header(title='Anomaly Ranges and respective accuracies per File')
    .tab_stub(groupname_col='grp',rowname_col='row')
).show()
```
...
## Looking at results for 1000 splits
...
For 1000 train-test splits, the model has scores which don't vary by a high 
degree.

This assumes that the data points are independent and we're looking at this
graph to determine if the scores obtained by previous models were just random
chance or does it cause similar scores for different splits where we ignore 
having to determine groups.


## Using the provided Train Files
...
There is no anomalies in these files and is not normally distributed meaning it
will most likely perform very poorly.
