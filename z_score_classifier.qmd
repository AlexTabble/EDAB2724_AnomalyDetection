...
class StatsModel(BaseEstimator, ClassifierMixin):
  ...
  No train,test splitting is involved
  """
  ...
  def _determine_IQR_mask(self, X : np.array) -> None:
    Q1 = np.quantile(X, 0.25)
    Q3 = np.quantile(X, 0.75)
    IQR = Q3 - Q1

    IQR_LB = np.mean(X) - self.iqr_threshold * IQR
    IQR_UB = np.mean(X) + self.iqr_threshold * IQR

    self.IQR_mask = (X > IQR_UB) | (X < IQR_LB)

  def _determine_mad_mask(self, X : np.array) -> None:
    MAD = np.sum(np.abs(X - np.mean(X)))  / len(X)
    median_diff = np.abs(X - np.median(X))

    self.MAD_mask = median_diff > MAD * self.mad_threshold

  def _determine_normal_z_mask(self, X : np.array) -> None:
    normal_z = np.abs((X - np.mean(X)) / np.std(X))
    self.normal_z_mask = normal_z > self.normal_z_threshold

  

  def _determine_rolling_z_mask(self, X :np.array) -> None:
    ...
    self.z_rolling_mask = np.abs(z_rolling > self.rolling_z_threshold)

  def fit(self, X : np.array, y=None) -> None:
    self._determine_mad_mask(X)
    self._determine_rolling_z_mask(X)
    self._determine_normal_z_mask(X)
    self._determine_IQR_mask(X)
    return self
  ...
  def predict(self, X = None) -> np.array:
    mask_df = pd.DataFrame({
      'IQR' : self.IQR_mask,
      "MAD" : self.MAD_mask,
      'Normal_z' : self.normal_z_mask,
      'Rolling_z' : self.z_rolling_mask
    })
  ...
  def score(self, X , y_true) -> float:
    ...
    #true_groups = Benchmarking.create_anomaly_groups(pd.Series(y_true),show_printout=False)
    #pred_groups = Benchmarking.create_anomaly_groups(pd.Series(y_pred),show_printout=False)

    return recall_score(y_true, y_pred)
    #return Benchmarking._evaluate_groups(pred_groups, true_groups,show_printout=False)
...
def create_benchmark_table(benchmark_df: pd.DataFrame) -> None:
  ...
  (
    GT(benchmark_df)
    .tab_header(title=md('Benchmark Results: **Stats Model**'),
                subtitle=md("results **before any hyperparameter tuning**"))
...
print('Time for 100 epochs', end-start)
...
def calculate_estimated_tuning_time(params: dict):
  total_iterations = 1
  for key, value in params.items():
    total_iterations *= len(value)
  print(total_iterations)
  print('Estimated Tuning Time', (total_iterations / 16), 'seconds')

calculate_estimated_tuning_time(param_grid)
...
```{python} 
best_params, best_score =grid_search(StatsModel(), combined['Value1'],combined['Labels'],param_grid)
...
```{python} 
benchmark_df = run_benchmark(model)
create_benchmark_table(benchmark_df)
```
